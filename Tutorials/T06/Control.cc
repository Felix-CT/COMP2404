...
void Control::generateMatches(MatchListByScore& scoreList, MatchListByClient& clientList){
  for (int i = 0; i < numAnimals, i++) {
    for (int j = 0; j < numClients; i++) {
      int randNum = rand() % 21;
      if (randNum > 0){
        Match* match = new Match(animals[i], clients[j]);
        scoreList.add(match);
...
void Control::launch(){
  ...
  initClientsTemp();
  initAnimalsTemp();
  MatchListByScore scoreList = MatchListByScore();
  MatchListByClient clientList = MatchListByClient();
  generateMatches(&scoreList, clientList);
  scoreList.print();
  clientList.print();
  scoreList.cleanup();
  clientList.cleanup()
